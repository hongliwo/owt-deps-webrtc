...
/*
 * testAAC.cpp : Defines the entry point for the console application.
 */
...
/* Runtime statistics */
#include <time.h>
#define CLOCKS_PER_SEC_AAC 100000
...
int main(int argc, char* argv[]) {
  ...
  AACEncInst* AACenc_inst;
  AACDecInst* AACdec_inst;
  ...
  if (argc != 5) {
    ...
    printf("\n");
    printf("Usage:\n\n");
    printf("./testAAC.exe framelength infile outbitfile outspeechfile \n\n");
    printf("with:\n");
  ...
  /* Create and init */
  WebRtcAAC_CreateEncoder((AACEncInst**)&AACenc_inst);
  WebRtcAAC_CreateDecoder((AACDecInst**)&AACdec_inst);
  WebRtcAAC_EncoderInit((AACEncInst*)AACenc_inst);
  WebRtcAAC_DecoderInit((AACDecInst*)AACdec_inst);
  ...
  while (!endfile) {
    ...
    starttime = clock() / (double)CLOCKS_PER_SEC_AAC;

    /* G.722 encoding + decoding */
    stream_len = WebRtcAAC_Encode((AACEncInst*)AACenc_inst, shortdata,
                                   framelength, streamdata);
    WebRtcAAC_Decode(AACdec_inst, streamdata, stream_len, decoded,
                      speechType);

    /* Stop clock after call to encoder and decoder */
    runtime += (double)((clock() / (double)CLOCKS_PER_SEC_AAC) - starttime);
  ...
  WebRtcAAC_FreeEncoder((AACEncInst*)AACenc_inst);
  WebRtcAAC_FreeDecoder((AACDecInst*)AACdec_inst);
